  ...
  function Header()
    {
      ...
      $this->SetXY(100, 10);
       ...
       $this->SetFillColor(180,180,255);
        ...
        $this->cell(60,5,'Worksapce Name',1,0,'',true);
...
foreach ($data as $value)
 {
     $stmt1 = $conn->prepare("SELECT * FROM workspaces where id='".$value['workspace_id']."'");
     $stmt1->execute();
     $result1 = $stmt1->setFetchMode(PDO::FETCH_ASSOC); 
      //$package=$stmt1->fetchAll();
      $package=$stmt1->fetch(PDO::FETCH_ASSOC);

     $stmt2 = $conn->prepare("SELECT * FROM clients where id='".$value['client_id']."'");
     $stmt2->execute();
     $result2 = $stmt2->setFetchMode(PDO::FETCH_ASSOC);
      ...
      $traveller=$stmt2->fetch(PDO::FETCH_ASSOC);
          ...
          $pdf->cell(60,5,$package['wname'],1,0);
